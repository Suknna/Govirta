...
| Public error alias | `client.go:13` | `qemuimg.ErrInvalidRequest = imgexec.ErrInvalidRequest` |
...
- Process failures route through `imgexec.WrapError(result, err)` → `*CommandError`, preserving stdout/stderr. JSON parse failures (info/check) bypass `WrapError` and return the raw `json` error directly — by design.
...
- Local chain:
  ...
  5. `<sub>.Builder.Do(ctx)` — validate non-empty / positive fields with `imgexec.InvalidRequest` (returns `%w ErrInvalidRequest`); on pass, assemble argv slice
  6. `<sub>.Builder.Do → b.runner.Run(ctx, b.binary, args)` — runner invocation:
     ...
     - Process / failure path: caller wraps via `imgexec.WrapError(result, err)` → `*CommandError` with `Unwrap()` exposing original error (success case returns `nil` because `WrapError(_, nil) == nil`)
...
- Side effects (within module): spawns `qemu-img` subprocess via `OSRunner` (default); `remove.Builder.Do` is the **exception** — it calls `os.Remove` directly without invoking the runner (`remove/remove.go:42` and `remove/remove.go:47`), enforcing `.qcow2` extension, checking `ctx.Err()` before stat, and rejecting directories at `remove/remove.go:35-45`
- Exit / next hop:
