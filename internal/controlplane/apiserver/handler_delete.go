...
func (s *Server) delete(ctx context.Context, r *http.Request) *apiError {
	...
	// spec/status are physically preserved on write-back (see deleteObject).
	obj, err := decodeDeleteObject(raw.Value)
	if err != nil {
...
// deleteObject is the minimal, kind-agnostic decode of a stored object for the
// delete path. The whole object is parsed only as a top-level map of raw bytes
// (rest), and metadata is parsed into the typed ObjectMeta so the handler can
// read/stamp deletionTimestamp and finalizers without bare-string fiddling. On
// re-marshal, every non-metadata key (spec, status, apiVersion, kind, ...) is
// carried through as its original bytes, so this handler physically cannot
// rewrite spec/status — mirroring the status patch pass-through design.
type deleteObject struct {
	Metadata metav1.ObjectMeta
	rest     map[string]json.RawMessage
}

// decodeDeleteObject parses raw stored bytes into a deleteObject: the top-level
// map preserves spec/status as opaque bytes, while metadata is decoded into the
// typed ObjectMeta. ObjectMeta is the canonical, complete metadata model (no
// fields outside it), so this round-trip is lossless for metadata too.
func decodeDeleteObject(value []byte) (deleteObject, error) {
	rest := map[string]json.RawMessage{}
	if err := json.Unmarshal(value, &rest); err != nil {
		return deleteObject{}, fmt.Errorf("decode stored object: %w", err)
	}
	var meta metav1.ObjectMeta
	if metaRaw, ok := rest["metadata"]; ok {
		if err := json.Unmarshal(metaRaw, &meta); err != nil {
			return deleteObject{}, fmt.Errorf("decode object metadata: %w", err)
		}
	}
	return deleteObject{Metadata: meta, rest: rest}, nil
}
...
// bytes, untouched.
func (o deleteObject) marshal() ([]byte, error) {
