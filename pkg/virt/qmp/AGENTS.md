...
Verified-against:
  ...
  files:
    ...
    - pkg/virt/qmp/errors.go
    - pkg/virt/qmp/integration_test.go
    - pkg/virt/qmp/internal/monitor/monitor.go
    - pkg/virt/qmp/internal/monitor/goqemu.go
    - pkg/virt/qmp/internal/goqemu/socket.go
    - pkg/virt/qmp/internal/status/status.go
    - pkg/virt/qmp/internal/power/power.go
    - pkg/virt/qmp/internal/events/events.go
    - pkg/virt/qmp/internal/protocol/error.go
  flows:
...
Project-owned QMP boundary. Callers import only `pkg/virt/qmp`; raw protocol transport, command JSON, and event conversion stay under `pkg/virt/qmp/internal/*`.
...
- Entry from root flow: `pkg/virt/qmp/client.go:52 (NewSocketClient)` / `:76 (SocketClient.Connect)` — future node runtime path from root `#flow-govirtlet-boot`
- Local chain:
  1. `pkg/virt/qmp/client.go:57 (newSocketClient)` — validate `Config.SocketPath`, monitor factory, default timeout
  2. `pkg/virt/qmp/client.go:81 (SocketClient.Connect)` — check context, take `lifecycleMu`, reject duplicate connection
  3. `pkg/virt/qmp/client.go:95 (SocketClient.Connect)` — call `factory.New("unix", socketPath, timeout)`
  4. `pkg/virt/qmp/internal/monitor/goqemu.go:14 (GoQEMUFactory.New)` — wrap vendored socket monitor
  5. `pkg/virt/qmp/internal/goqemu/socket.go:93 (NewSocketMonitor)` — dial QMP unix socket
  6. `pkg/virt/qmp/internal/goqemu/socket.go:108 (SocketMonitor.Connect)` — read greeting, send `qmp_capabilities`, start listener
  7. `pkg/virt/qmp/client.go:120 (SocketClient.Connect)` — install connected monitor and reset event state
...
- Entry from root flow: `pkg/virt/qmp/client.go:163 (SocketClient.QueryStatus)` / `:176 (SystemPowerdown)` / `:185 (Quit)`
- Local chain:
  1. `pkg/virt/qmp/client.go:230 (connectedMonitor)` — read active monitor or return `ErrNotConnected`
  2. `pkg/virt/qmp/internal/status/status.go:23 (Query)` — marshal `{"execute":"query-status"}` and call monitor
  3. `pkg/virt/qmp/internal/power/power.go:18 (SystemPowerdown)` / `:23 (Quit)` — marshal power command and call monitor
  4. `pkg/virt/qmp/internal/goqemu/socket.go:210 (SocketMonitor.Run)` — write command JSON, wait for response, decode protocol errors
  5. `pkg/virt/qmp/client.go:172 (SocketClient.QueryStatus)` — convert internal status to public `Status`, preserving unknown state text
...
- Entry from root flow: `pkg/virt/qmp/client.go:194 (SocketClient.Events)`
- Local chain:
  1. `pkg/virt/qmp/client.go:194 (SocketClient.Events)` — require connected monitor and reject duplicate stream
  2. `pkg/virt/qmp/client.go:204 (SocketClient.Events)` — create per-stream child context
  3. `pkg/virt/qmp/internal/goqemu/socket.go:295 (SocketMonitor.Events)` — expose underlying QMP event channel
  4. `pkg/virt/qmp/internal/events/events.go:18 (Stream)` — convert/filter raw monitor events by event name
  5. `pkg/virt/qmp/client.go:250 (convertEvents)` — convert internal event to public `qmp.Event`
  6. `pkg/virt/qmp/client.go:140 (Disconnect)` — cancels connection-owned event context on disconnect
...
- `go test ./pkg/virt/qmp/...` covers unit behavior without live QEMU. The live QMP test is opt-in through `GOVIRTA_QMP_INTEGRATION=1` and `GOVIRTA_QMP_SOCKET`.
