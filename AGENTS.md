...
Verified-against:
  ...
  files:
    ...
    - internal/govirtctl/client.go
    - internal/govirtctl/command.go
...
| 快照 Snapshot | `snapshotv1.Snapshot` + `controllers.SnapshotController`（`pkg/apis/snapshot/v1alpha1/types.go` / `internal/node/controllers/snapshot.go`） | whole-VM 冷快照 API；控制面从目标 VM 派生 nodeName，节点在 VM 冷态下 fan-out 到每个 qcow2 内部快照。 |
...
├── internal/controlplane/AGENTS.md  # control plane: etcd store + apiserver + MAC allocator + scheduler
│   └── internal/controlplane/apiserver/AGENTS.md # HTTP API + admission + CAS/delete/finalizers
├── internal/network/AGENTS.md       # VM-facing network orchestration layer (netpool + services)
├── internal/node/AGENTS.md          # compute node agent: controller-manager + 7 per-resource controllers
│   └── internal/node/controllers/AGENTS.md # per-resource reconcilers + cold/finalizer flows
...
├── pkg/apis/AGENTS.md               # API contract types (7 resource kinds + shared meta envelope)
...
├── test/e2e/AGENTS.md               # distributed spine E2E topology + manifest lifecycle
...
│   ├── apiserver/       # API server boundary（旧 noop skeleton，已被 controlplane/apiserver 取代）
│   ├── controlplane/    # control plane composition (详见 internal/controlplane/AGENTS.md)
│   │   ├── apiserver/   # real HTTP apiserver: apply/replace/get/list/watch/status/delete/finalizers (详见 internal/controlplane/apiserver/AGENTS.md)
│   │   ├── store/       # Store interface + etcd/fake implementations
│   │   ├── mac/         # MAC pool + allocator (etcd-backed)
│   │   └── scheduler/   # Scheduler interface + NoopScheduler
│   ├── govirtctl/       # CLI client library (apply/get HTTP client + command dispatch)
│   ├── network/         # VM-facing network orchestration layer (详见 internal/network/AGENTS.md)
│   ├── node/            # compute node agent composition (详见 internal/node/AGENTS.md)
│   │   ├── client/      # HTTP client to master + watch source
│   │   ├── controller/  # controller-manager framework (Manager, Queue, loop)
│   │   ├── controllers/ # 7 per-resource reconcilers (storagepool/image/volume/network/nic/vm/snapshot; 详见 internal/node/controllers/AGENTS.md)
│   │   └── identity/    # deterministic kernel identity derivation (nftables/TAP names)
│   ├── scheduler/       # placement boundary (old location, superseded by controlplane/scheduler)
...
│   ├── apis/            # API contract types (详见 pkg/apis/AGENTS.md)
│   │   ├── meta/        # shared TypeMeta + ObjectMeta envelope
│   │   ├── image/       # Image types
│   │   ├── network/     # Network types
│   │   ├── nic/         # NIC types
│   │   ├── storagepool/ # StoragePool types
│   │   ├── vm/          # VM types
...
├── scripts/
│   ├── verify.sh        # 本地 CI 等价验证入口
│   ├── acceptance.sh    # Lima Linux acceptance runner
│   └── e2e.sh           # E2E distributed spine test runner
└── test/
    ├── acceptance/      # Linux-only Lima acceptance tests
    └── e2e/             # E2E closure test (etcd + govirtad + Lima govirtlet)
```
...
| Task | Location | Notes |
| --- | --- | --- |
| 控制面入口 | `cmd/govirtad/main.go` → `internal/controlplane/service.go` → `internal/controlplane/apiserver/server.go` | etcd store + MAC allocator + scheduler + real HTTP apiserver |
| 控制面 HTTP API | `internal/controlplane/apiserver/` (详见 `internal/controlplane/apiserver/AGENTS.md`) | apply/replace/get/list/watch/status/delete/finalizers handlers for 7 resource kinds |
| 控制面持久化 | `internal/controlplane/store/` (详见 `internal/controlplane/AGENTS.md`) | `Store` interface; etcd + fake implementations |
| 控制面 MAC 分配 | `internal/controlplane/mac/` (详见 `internal/controlplane/AGENTS.md`) | `MACAllocator` interface; etcd-backed, mutex-serialized |
| 控制面调度 | `internal/controlplane/scheduler/` (详见 `internal/controlplane/AGENTS.md`) | `Scheduler` interface; `NoopScheduler` picks first node |
| 节点入口 | `cmd/govirtlet/main.go` → `internal/node/agent.go` → controller-manager → 7 controllers | (详见 `internal/node/AGENTS.md`) |
| 节点控制器框架 | `internal/node/controller/` (详见 `internal/node/AGENTS.md`) | Manager + Queue + reconcile loop; self-built, k8s-client-go-free |
| 节点资源控制器 | `internal/node/controllers/` (详见 `internal/node/controllers/AGENTS.md`) | StoragePool/Image/Volume/Network/NIC/VM/Snapshot reconcilers |
| CLI 入口 | `cmd/govirtctl/main.go` → `internal/govirtctl/` | `apply`/`replace`/`delete`/`get`/`version` commands; kind-agnostic manifest transport |
| API 类型 | `pkg/apis/` (详见 `pkg/apis/AGENTS.md`) | 7 resource kinds + shared meta envelope; no internal imports |
| E2E 测试 | `test/e2e/` + `scripts/e2e.sh` (详见 `test/e2e/AGENTS.md`) | distributed spine closure: etcd + govirtad + Lima govirtlet + lifecycle/cold-operation checks |
...
| qemu-img | `pkg/virt/qemuimg/` (详见 `pkg/virt/qemuimg/AGENTS.md`) | Create/Info/Convert/Resize/Snapshot/SnapshotDelete/SnapshotList/Check/Remove + runner |
...
| --- | --- | --- | --- |
| `main` | func | `cmd/govirtad/main.go:34` | 解析 flags → `controlplane.Config` → `NewService` → `Run` |
| `controlplane.Config` | struct | `internal/controlplane/service.go:33` | etcd endpoints、listen addr、MAC pool、node names |
| `controlplane.NewService` | func | `internal/controlplane/service.go:71` | dial etcd → assemble mac/scheduler/apiserver |
| `controlplane.Service.Run` | method | `internal/controlplane/service.go:109` | `apiServer.Run(ctx)` + `store.Close()` |
| `apiserver.Server` | struct | `internal/controlplane/apiserver/server.go:34` | HTTP surface; holds store/alloc/sched/nodeNames |
| `apiserver.Server.Handler` | method | `internal/controlplane/apiserver/server.go:68` | 注册 POST/PUT/GET/PATCH 路由 |
| `apiserver.Server.Apply` | handler | `internal/controlplane/apiserver/handler_apply.go:60` | kind-dispatched apply 管线 |
| `apiserver.Server.Watch` | handler | `internal/controlplane/apiserver/handler_watch.go:60` | chunked NDJSON 流式 watch |
| `apiserver.Server.PatchStatus` | handler | `internal/controlplane/apiserver/handler_status.go:42` | read-modify-write CAS status 合并 |
| `store.Store` | interface | `internal/controlplane/store/store.go:55` | `Put`/`Get`/`List`/`Delete`/`Watch`/`Close` |
| `store.RunStoreContract` | func | `internal/controlplane/store/contract.go:17` | 可复用行为验证套件 |
| `mac.MACAllocator` | interface | `internal/controlplane/mac/allocator.go:29` | `WithAllocation(ctx, commit func(mac) error)` |
| `scheduler.Scheduler` | interface | `internal/controlplane/scheduler/scheduler.go:32` | `Schedule(ctx, VM, []string) (string, error)` |
| `main` | func | `cmd/govirtlet/main.go:20` | 解析 flags → `node.Config` → `NewAgent` → `Run` |
| `node.Config` | struct | `internal/node/agent.go:41` | MasterURL、NodeName、RuntimeRoot、OwnerUID/GID、GuestCPU |
| `node.Agent.Run` | method | `internal/node/agent.go:143` | 启动 controller-manager，7 个 per-resource controller |
| `controller.Manager` | struct | `internal/node/controller/manager.go:26` | hosts controllers + shared EventSource |
| `controller.Controller` | interface | `internal/node/controller/controller.go:42` | `Kind()` + `Reconcile(ctx, Event)` |
| `controller.Queue` | struct | `internal/node/controller/queue.go:10` | deduplicating FIFO workqueue |
| `node/client.Client` | struct | `internal/node/client/client.go:28` | HTTP client to master: Get/List/PatchStatus |
| `node/client.WatchSource` | struct | `internal/node/client/watch.go:21` | NDJSON stream → Event channel |
| `govirtctl.Client` | struct | `internal/govirtctl/client.go:27` | CLI HTTP client: Apply/Get |
| `govirtctl.Run` | func | `internal/govirtctl/command.go:27` | CLI entrypoint: apply/get/version dispatch |
...
- Trigger: `cmd/govirtad/main.go:34 (main)` (process entry; parses flags into `controlplane.Config`)
- Cross-module chain:
  1. `cmd/govirtad/main.go:35 (main)` — 构造 zerolog logger（`process=govirtad`）
  2. `cmd/govirtad/main.go:36 (main)` — `logger.WithContext(context.Background())` 得到 root `ctx`
  3. `cmd/govirtad/main.go:38 (main → parseConfig)` — 解析 flags → `controlplane.Config{EtcdEndpoints, ListenAddr, MACPrefix/Start/End, NodeNames}`
  4. `cmd/govirtad/main.go:44 (main → controlplane.NewService)` — 进入 `internal/controlplane` 装配层 [详见 `internal/controlplane/AGENTS.md#flow-controlplane-boot`]
  5. `internal/controlplane/service.go:72 (NewService → etcd.New)` — dial etcd 集群，构造 `*etcd.Store`
  6. `internal/controlplane/service.go:80 (newServiceWithStore)` — 组装 `mac.NewPool` + `mac.NewAllocator` + `scheduler.NewNoopScheduler` + `apiserver.NewServer`
  7. `internal/controlplane/service.go:109 (Service.Run)` → `apiserver.Server.Run(ctx)` — `net.Listen` → HTTP accept loop
- Data: flags → `controlplane.Config` → `*etcd.Store` + `*mac.Pool` + `*etcdAllocator` + `*NoopScheduler` + `*apiserver.Server`
- Boundaries: etcd gRPC connection; TCP listener for HTTP API
- Sinks: etcd cluster (持久化)、TCP socket (HTTP API)

### Flow: govirtlet node agent boot {#flow-govirtlet-boot}

- Trigger: `cmd/govirtlet/main.go:20 (main)` (process entry on compute host)
- Cross-module chain:
  1. `cmd/govirtlet/main.go:21 (main)` — 构造 zerolog logger（`process=govirtlet`）
  2. `cmd/govirtlet/main.go:22 (main)` — `logger.WithContext(context.Background())` 得到 root `ctx`
  3. `cmd/govirtlet/main.go:24 (main → parseConfig)` — 解析 flags → `node.Config{MasterURL, NodeName, RuntimeRoot, ImageSourceRoot, OwnerUID/GID, GuestCPU}`
  4. `cmd/govirtlet/main.go:30 (main → node.NewAgent)` — 进入 `internal/node` 组合层 [详见 `internal/node/AGENTS.md#flow-node-boot`]
  5. `internal/node/agent.go:90 (NewAgent → buildHostManagers)` — Linux: 构造真实 netlink/nftables/CoreDHCP/exec managers
  6. `internal/node/agent.go:95-106 (NewAgent)` — 构造 HTTP client、WatchSource、pool/volume/image/network/nic/vmm services
  7. `internal/node/agent.go:122-130 (NewAgent)` — 构造 7 个 per-resource controllers
  8. `internal/node/agent.go:143 (Agent.Run)` → `controller.Manager.Run` — 每个 controller 启动一个 goroutine [详见 `internal/node/AGENTS.md#flow-node-reconcile`]
- Data: flags → `node.Config` → `hostManagers` + `client.Client` + `WatchSource` + domain services + 7 controllers + `controller.Manager`
- Boundaries: HTTP 连接 master apiserver；goroutine per controller
- Sinks: master apiserver (Get/List/PatchStatus)；host 原语 (netlink/nftables/CoreDHCP)；QEMU 进程 (via vmm)
...
### Flow: control plane apply API {#flow-apiserver-apply}

- Trigger: `internal/controlplane/apiserver/handler_apply.go:47 (Server.Apply)` (HTTP POST `/apis/{kind}/{name}`)
- Cross-module chain:
  1. `cmd/govirtad/main.go:44 (main → controlplane.NewService)` — 装配 apiserver [详见 `internal/controlplane/AGENTS.md#flow-controlplane-boot`]
  2. `internal/controlplane/apiserver/handler_apply.go:68 (Server.apply)` — kind-dispatched decode→admission→store [详见 `internal/controlplane/apiserver/AGENTS.md#flow-apiserver-apply`]
  3. `internal/controlplane/apiserver/handler_apply.go:318 (applyNIC)` — NIC: 空 MAC → `mac.Allocator.WithAllocation` 原子分配 [详见 `internal/controlplane/AGENTS.md#flow-apiserver-apply`]
  4. `internal/controlplane/apiserver/handler_apply.go:418 (bindVM)` — VM: 无显式 node → `scheduler.Schedule` 落地
  5. `internal/controlplane/apiserver/handler_apply.go:383 (applySnapshot)` — Snapshot: 从目标 VM 派生 nodeName
  6. `internal/controlplane/apiserver/handler_apply.go:433 (Server.put)` → `store.Put(ctx, key, data, "")`
- Data: HTTP body `[]byte` → typed API object → `json.Marshal` → etcd key-value
- Boundaries: HTTP request → etcd write
- Sinks: etcd cluster；MAC allocation (NIC)；node binding (VM/Snapshot)

### Flow: node agent reconciliation {#flow-node-reconcile}

- Trigger: `internal/node/agent.go:143 (Agent.Run)` → `controller.Manager.Run` (one goroutine per resource kind)
- Cross-module chain:
  1. `cmd/govirtlet/main.go:30 (main → node.NewAgent)` — 装配 7 个 controllers [详见 `internal/node/AGENTS.md#flow-node-boot`]
  2. `internal/node/controller/loop.go:21 (runController)` — feeder goroutine + reconcile loop [详见 `internal/node/AGENTS.md#flow-node-reconcile`]
  3. `internal/node/client/watch.go:71 (WatchSource.Watch)` — HTTP NDJSON watch stream from master apiserver
  4. `internal/node/controllers/*.go` — per-resource `Reconcile`: decode → guard → gate/cold gate → domain work → patchStatus/removeFinalizer [详见 `internal/node/controllers/AGENTS.md`]
  5. `internal/node/client/client.go:113 (Client.PatchStatus)` — PATCH `/apis/{kind}/{name}/status` back to master
- Data: HTTP NDJSON → `controller.Event` → typed API object → domain service → status JSON → HTTP PATCH
- Boundaries: HTTP watch stream (master→node)；HTTP status patch (node→master)
- Sinks: master apiserver (status updates)；host primitives (storage/network/vmm domain work)

### Flow: control plane replace API {#flow-apiserver-replace}

- Trigger: `internal/controlplane/apiserver/handler_replace.go:26 (Server.Replace)` (HTTP PUT `/apis/{kind}/{name}`)
- Cross-module chain:
  1. `internal/controlplane/apiserver/server.go:70 (Handler)` — register PUT handler [详见 `internal/controlplane/apiserver/AGENTS.md#flow-apiserver-replace`]
  2. `internal/controlplane/apiserver/handler_replace.go:118 (decodeAndAdmitReplace)` — decode submitted object, require `metadata.resourceVersion`, run replace admission
  3. `internal/controlplane/apiserver/handler_replace.go:175 (putReplaceResponse)` — `store.Put(ctx, key, data, expectedVersion)` CAS [详见 `internal/controlplane/AGENTS.md#flow-apiserver-replace`]
- Data: edited object JSON → admitted replacement JSON → etcd conditional write
- Boundaries: HTTP request → etcd compare-and-swap
- Sinks: etcd key update; stale resourceVersion returns HTTP 409

### Flow: delete and finalizer drain {#flow-apiserver-delete-finalizers}

- Trigger: `internal/controlplane/apiserver/handler_delete.go:48 (Server.Delete)` and `internal/controlplane/apiserver/handler_finalizers.go:54 (Server.PatchFinalizers)`
- Cross-module chain:
  1. `internal/controlplane/apiserver/handler_delete.go:73 (Server.delete)` — reverse-reference guard, stamp `metadata.deletionTimestamp`, preserve finalizers [详见 `internal/controlplane/apiserver/AGENTS.md#flow-apiserver-delete-finalizers`]
  2. `internal/node/controllers/teardown.go:77 (isDeleting)` — node sees ordinary routed object with deletionTimestamp [详见 `internal/node/controllers/AGENTS.md#flow-node-finalizer-teardown`]
  3. `internal/node/client/client.go:146 (Client.RemoveFinalizer)` — node removes teardown finalizer after local cleanup
  4. `internal/controlplane/apiserver/handler_finalizers.go:151 (Server.patchFinalizers)` — last finalizer + deletionTimestamp → `store.DeleteIfVersion`
- Data: live object JSON → deleting object JSON → finalizer patch body → etcd tombstone delete
- Boundaries: HTTP DELETE, watch stream, HTTP PATCH finalizers, etcd conditional delete
- Sinks: node-local teardown + etcd delete

### Flow: Snapshot cold lifecycle {#flow-node-snapshot-cold-lifecycle}

- Trigger: `internal/node/controllers/snapshot.go:72 (SnapshotController.Reconcile)` (Snapshot routed to target VM node)
- Cross-module chain:
  1. `internal/controlplane/apiserver/handler_apply.go:383 (applySnapshot)` — control plane derives Snapshot nodeName from target VM [详见 `internal/controlplane/apiserver/AGENTS.md#flow-apiserver-apply-snapshot`]
  2. `internal/node/controllers/snapshot.go:303 (vmIsCold)` — live vmm cold gate [详见 `internal/node/controllers/AGENTS.md#flow-node-snapshot-cold-lifecycle`]
  3. `internal/storage/service.go:268 (VolumeService.SnapshotVolume)` / `:291 (DeleteVolumeSnapshot)` — per-volume fan-out [详见 `internal/storage/AGENTS.md#flow-storage-snapshot`]
  4. `pkg/virt/qemuimg/client.go:125 (QCOW2Client.Snapshot)` / `:130 (SnapshotDelete)` — qemu-img internal snapshot command [详见 `pkg/virt/qemuimg/AGENTS.md#flow-qcow2-do`]
- Data: `Snapshot{spec.vmRef}` → target VM volumes → per-qcow2 snapshot tags → Snapshot status/finalizer
- Boundaries: apiserver watch → node reconcile → qemu-img subprocess
- Sinks: qcow2 internal snapshot metadata; Snapshot status/finalizer updates

### Flow: Volume cold resize {#flow-node-volume-cold-resize}

- Trigger: `internal/node/controllers/volume_resize.go:28 (VolumeController.reconcileResize)` (ready Volume desired capacity drift)
- Cross-module chain:
  1. `internal/node/controllers/volume_resize.go:47 (vmIsCold)` — live cold gate [详见 `internal/node/controllers/AGENTS.md#flow-node-volume-cold-resize`]
  2. `internal/storage/service.go:242 (VolumeService.ResizeVolume)` — explicit absolute capacity request [详见 `internal/storage/AGENTS.md#flow-storage-resize`]
  3. `internal/storage/local/driver.go:438 (Driver.Resize)` — qemu-img info then grow-only resize
  4. `pkg/virt/qemuimg/client.go:110 (QCOW2Client.Info)` / `:120 (Resize)` — runner boundary [详见 `pkg/virt/qemuimg/AGENTS.md#flow-qcow2-do`]
- Data: `Volume.spec.capacityBytes` → live qcow2 virtual size → updated volume capacity
- Boundaries: node reconcile → storage service/driver → qemu-img subprocess
- Sinks: qcow2 virtual size growth; in-memory pool accounting update

### Flow: VM cold config change {#flow-vm-cold-config-change}

- Trigger: `internal/node/controllers/vm_config.go:53 (VMController.reconcileConfigDrift)` (VM observed off with spec drift)
- Cross-module chain:
  1. `internal/controlplane/apiserver/admission/fields.go:120 (VMColdMutableFields)` — replace admission allows cold-mutable fields only with submitted `powerState=Off` [详见 `internal/controlplane/apiserver/AGENTS.md#flow-apiserver-replace`]
  2. `internal/node/controllers/vm.go:281 (reconcileExistingVMOff)` — only off path checks config drift
  3. `internal/node/controllers/vm_config.go:74 (vmm.Redefine)` — rewrite persisted VMM spec/argv [详见 `internal/node/controllers/AGENTS.md#flow-vm-cold-config-change`]
  4. `internal/vmm/redefine.go:25 (VMMService.Redefine)` — vm.json rewrite only, no process mutation [详见 `internal/vmm/AGENTS.md#flow-vmm-redefine`]
- Data: replaced `VM.spec` → `vmm.SpecSummary` → persisted `vm.json` + argv
- Boundaries: apiserver admission/CAS → node reconcile → filesystem state write
- Sinks: node runtime `vm.json` config; next Start uses new argv
...
### Flow: storage qcow2 snapshot {#flow-storage-snapshot}

- Trigger: `internal/storage/service.go:268 (VolumeService.SnapshotVolume)` / `:291 (DeleteVolumeSnapshot)`
- Cross-module chain:
  1. `internal/storage/service.go:268 (SnapshotVolume)` — validate explicit pool, volume ID, snapshot name [详见 `internal/storage/AGENTS.md#flow-storage-snapshot`]
  2. `internal/storage/pool/service.go:398 (Service.SnapshotVolume)` — lookup existing volume
  3. `internal/storage/local/driver.go:357 (Driver.Snapshot)` / `:385 (DeleteSnapshot)` — resolve owned qcow2 path and call qemu-img
  4. `pkg/virt/qemuimg/client.go:125 (QCOW2Client.Snapshot)` / `:130 (SnapshotDelete)` / `:142 (SnapshotList)` [详见 `pkg/virt/qemuimg/AGENTS.md#flow-qcow2-do`]
- Data: `SnapshotVolumeRequest` → `block.SnapshotRequest` → qcow2 internal snapshot tag
- Boundaries: in-proc storage call → qemu-img subprocess
- Sinks: qcow2 internal snapshot metadata; no storage metadata record

### Flow: storage cold resize {#flow-storage-resize}

- Trigger: `internal/storage/service.go:242 (VolumeService.ResizeVolume)`
- Cross-module chain:
  1. `internal/storage/service.go:242 (ResizeVolume)` — validate explicit pool, volume ID, absolute target capacity [详见 `internal/storage/AGENTS.md#flow-storage-resize`]
  2. `internal/storage/pool/resize.go:22 (Service.ResizeVolume)` — reserve capacity delta under pool lock
  3. `internal/storage/local/driver.go:438 (Driver.Resize)` — compare live virtual size, never shrink
  4. `pkg/virt/qemuimg/client.go:110 (QCOW2Client.Info)` / `:120 (Resize)` [详见 `pkg/virt/qemuimg/AGENTS.md#flow-qcow2-do`]
- Data: `ResizeVolumeRequest` → live qcow2 virtual size → updated `volume.Volume.CapacityBytes`
- Boundaries: in-proc storage call → qemu-img subprocess
- Sinks: qcow2 virtual size growth; in-memory pool capacity update after driver success
...
### Flow: distributed spine E2E closure {#flow-e2e-distributed-spine}

- Trigger: `scripts/e2e.sh:301 (run_e2e)` starts `go test -v -tags e2e -count=1 ./test/e2e/...`
- Cross-module chain:
  1. `scripts/e2e.sh:301 (run_e2e)` — Docker etcd + host govirtad + Lima govirtlet topology [详见 `test/e2e/AGENTS.md#flow-e2e-distributed-spine`]
  2. `test/e2e/closure_test.go:102 (TestDistributedSpineClosure)` — apply dependency manifests and boot VM
  3. `test/e2e/closure_test.go:255 (snapshotColdCycle)` — verify qcow2 snapshot live in guest
  4. `test/e2e/closure_test.go:281 (coldResizeVolume)` — verify guest qcow2 virtual size
  5. `test/e2e/closure_test.go:330 (coldConfigChange)` — verify next QEMU argv reflects cold config
  6. `test/e2e/closure_test.go:772 (teardownSpine)` — delete resources and assert no guest orphans
- Data: manifests JSON → `govirtctl` HTTP calls → master watch → node reconcile → live guest/qcow2/QEMU assertions
- Boundaries: host shell, Docker etcd, host HTTP server, Lima guest govirtlet, QEMU/qemu-img inside guest
- Sinks: real etcd keys, guest qcow2 files, QEMU process, node-local network/storage/vmm state
...
# Distributed spine E2E verification
scripts/e2e.sh check-tools
scripts/e2e.sh full
```
...
- This file has been conservatively refreshed through `dfad16b`: DHCP/network orchestration knowledge remains, and Snapshot, delete/finalizers, replace/CAS, cold resize/config, VMM redefine, and distributed-spine E2E flows are now indexed.
...
- 一等公民与可观测性：`## FIRST-CLASS CITIZENS（一等公民）` 显式收录被编排的领域资源实体（Storage Pool / Volume / Image / Snapshot / Network / NIC / VM / Host Net Primitives / Node·Control Plane）与横切的可观测性；`## OBSERVABILITY（可观测性 — 强制规约）` 定义日志/指标/追踪三支柱 + 节点资源汇报的强制细则（A1 契约优先、OTel 为目标后端、`Meter`/`Tracer` 抽象待引入）。可观测性指标含节点级与 VM 级 CPU/内存/存储/网络基础资源使用。
