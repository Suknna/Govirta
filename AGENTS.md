...
├── internal/node/AGENTS.md          # compute node agent: controller-manager + 6 per-resource controllers
...
├── pkg/apis/AGENTS.md               # API contract types (6 resource kinds + shared meta envelope)
...
│   │   ├── apiserver/   # real HTTP apiserver: apply/get/list/watch/status handlers
...
│   │   ├── controllers/ # 6 per-resource reconcilers (storagepool/image/volume/network/nic/vm)
...
| 控制面 HTTP API | `internal/controlplane/apiserver/` (详见 `internal/controlplane/AGENTS.md`) | apply/get/list/watch/status handlers for 6 resource kinds |
...
| 节点入口 | `cmd/govirtlet/main.go` → `internal/node/agent.go` → controller-manager → 6 controllers | (详见 `internal/node/AGENTS.md`) |
| 节点控制器框架 | `internal/node/controller/` (详见 `internal/node/AGENTS.md`) | Manager + Queue + reconcile loop; self-built, k8s-client-go-free |
| 节点资源控制器 | `internal/node/controllers/` (详见 `internal/node/AGENTS.md`) | StoragePool/Image/Volume/Network/NIC/VM reconcilers |
| CLI 入口 | `cmd/govirtctl/main.go` → `internal/govirtctl/` | `apply`/`get`/`version` commands; kind-agnostic manifest transport |
| API 类型 | `pkg/apis/` (详见 `pkg/apis/AGENTS.md`) | 6 resource kinds + shared meta envelope; no internal imports |
| E2E 测试 | `test/e2e/` + `scripts/e2e.sh` | distributed spine closure: etcd + govirtad + Lima govirtlet |
...
| `node.Agent.Run` | method | `internal/node/agent.go:143` | 启动 controller-manager，6 个 per-resource controller |
...
- Cross-module chain:
  ...
  7. `internal/node/agent.go:111-119 (NewAgent)` — 构造 6 个 per-resource controllers
...
- Data: flags → `node.Config` → `hostManagers` + `client.Client` + `WatchSource` + domain services + 6 controllers + `controller.Manager`
...
- Trigger: `internal/controlplane/apiserver/handler_apply.go:60 (Server.Apply)` (HTTP POST/PUT `/apis/{kind}/{name}`)
- Cross-module chain:
  ...
  2. `internal/controlplane/apiserver/handler_apply.go:81 (Server.apply)` — kind-dispatched decode→validate→admit→store
  3. `internal/controlplane/apiserver/handler_apply.go:167 (applyNIC)` — NIC: 空 MAC → `mac.Allocator.WithAllocation` 原子分配 [详见 `internal/controlplane/AGENTS.md#flow-apiserver-apply`]
  4. `internal/controlplane/apiserver/handler_apply.go:185 (bindVM)` — VM: 无显式 node → `scheduler.Schedule` 落地
  5. `internal/controlplane/apiserver/handler_apply.go:273 (Server.put)` → `store.Put(ctx, key, data, "")`
- Data: HTTP body `[]byte` → typed API object → `json.Marshal` → etcd key-value
- Boundaries: HTTP request → etcd write
- Sinks: etcd cluster；MAC allocation (NIC)；node binding (VM)
...
- Cross-module chain:
  1. `cmd/govirtlet/main.go:30 (main → node.NewAgent)` — 装配 6 个 controllers [详见 `internal/node/AGENTS.md#flow-node-boot`]
  ...
  4. `internal/node/controllers/*.go` — per-resource `Reconcile`: decode → guard → gate → domain work → patchStatus
